...
ZOC = (function () {
  ...
  // Updates one item. Throws on rate-limit (so the queue can back off).
  async function updateOne(item) {
    ...
    // Found nothing in any source: record it so the daily pass leaves it alone,
    // but do NOT clobber an existing count in extra.
  ...
  async function runQueue(items, label) {
    ...
    _running = true;
    _cancel = false;
    let done = 0, ok = 0, blocked = false;
    try {
      for (const item of items) {
        if (_cancel) break;
        let attempt = 0, backoff = 5000;
        while (true) {
          try {
      ...
      await saveStateNow();
      refreshTree();
      const tail = blocked ? " (paused: source rate-limited)" : "";
      notify(label + ": updated " + ok + "/" + items.length + tail);
    }
  }
  ...
  function addToWindow(window) {
    ...
    mk("zoc-tools-runall", "Update all stale citations now (Open Citations)",
  ...
  function removeFromWindow(window) {
    if (!window || !window.document) return;
    const doc = window.document;
    for (const id of ["zoc-item-update", "zoc-collection-update", "zoc-tools-runall"]) {
